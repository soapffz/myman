...
CREATE TABLE if not exists `searchengine_asset` (
    ...
    `web_weight` bigint(20) unsigned COMMENT '网站权重',
...
alter table
    searchengine_asset
add
    unique index_searchengine(ip, port);

INSERT
    IGNORE INTO `searchengine_asset`(
        `id`,
        `ip`,
        `port`,
        `rootdomain`,
